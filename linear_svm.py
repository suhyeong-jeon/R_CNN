...
from custom_hard_negative_mining_dataset import CustomHardNegativeMiningDataset
...
# model로 AlexNet에 우리가 학습시킨 모델이 적용된것이 들어감
        ...
        for phase in ['train', 'val']:
            ...
            for inputs, labels, cache_dicts in data_loaders[phase]:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    outputs = model(inputs) # output이 모델을 통해 입력 이미지에 대한 특징 벡터(4000*2096)을 얻는 부분
                    # print(outputs.shape)
                    _, preds = torch.max(outputs, 1)
                    loss = criterion(outputs, labels) # SVM 모델 학습시 사용되는 Hinge Loss를 통해 binary class를 판단하게 됨(배경, 차)
...
if __name__ == '__main__':
    ...
    data_loaders, data_sizes = load_data('./my_voc2007/classifier_car')
